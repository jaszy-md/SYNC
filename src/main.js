...
const selected=[0,1];
let stage,accumulator=0,lastTime=0,hintSeconds=0;
const state=new GameState(()=>ui.render());
function start(){input.clear();accumulator=0;hintSeconds=0;hintToast.hidden=true;stage=manager.load(1,selected.map(i=>CHARACTERS[i]));ui.resetPanel();state.set(State.PLAYING);}
function resume(){ui.resetPanel();state.set(State.PLAYING);}
...
document.querySelector('.brand').addEventListener('click',event=>{
  ...
  if(state.current===State.PLAYING)pause();else if(state.current!==State.PAUSED){ui.resetPanel();state.set(State.MENU);}
});
  ...
  if(state.current===State.PLAYING){
    ...
    if(accumulator>=1/120){
      ...
      while(accumulator>=1/120){
        ...
        if(requestedHint&&!stage.helpMarker){hintToast.textContent=requestedHint.text;hintSeconds=10;}
        accumulator-=1/120;actions.forEach(a=>{a.jump=false;a.interact=false;});
        if(stage.complete){state.set(State.STAGE_COMPLETE);break;}
      }
      input.endFrame();
    }
    ...
    if(debug){ctx.fillStyle='#050912e8';ctx.fillRect(12,12,900,95);ctx.fillStyle='#fff';ctx.font='14px monospace';[
