...
import {Platform,Door,Trigger,HelpMarker,PressurePlate} from '../objects.js';
...
export class Stage1 {
  constructor(characters) {
    this.players=characters.map((c,i)=>new Player(i,c,{x:70+i*65,y:554},i===0?
      {jumpSpeed:760,operateWinch:true,role:'Explorer'}:
      {jumpSpeed:500,carryCell:true,role:'Tech'}));
    ...
    this.cell={x:185,y:572,w:24,h:26,state:'LOOSE'};
    ...
    this.phase='ENTRY';this.charge=0;this.time=0;this.complete=false;this.helpMarker=null;this.message='';this.ping=null;
  }
  ...
  update(dt,inputs) {
    ...
    this.winch.active=this.phase!=='ENTRY'&&near(explorer,this.winch,12)&&inputs[explorer.id].interactHeld;
    this.setGate(this.gateA,this.phase!=='ENTRY'||this.plate.active);
    this.setGate(this.gateB,this.phase==='CHARGE'||this.phase==='EXIT'||this.winch.active);
    ...
    if(this.phase==='CHARGE'){
      ...
      if(this.charge>=2.5){this.phase='EXIT';this.door.unlock();}
    }
  ...
  interact(player){
    ...
    if(near(player,this.socketA,18)){
      if(this.cell.state==='CARRIED'&&this.phase!=='CHARGE'&&this.phase!=='EXIT'){
  ...
  hintPoint(){
    ...
    if(this.phase==='CHARGE')return {id:'charge',x:1017,y:559,text:'Ga op jullie eigen gemarkeerde vloercontact staan: P1 links, P2 rechts. Houd allebei interactie vast tot de ring helemaal gevuld is. Daarna gaan jullie samen naar de deur.'};
    return {id:'exit',x:1135,y:559,text:'De deur heeft stroom. Kom allebei dicht bij de uitgang en houd samen interactie vast.'};
  }
  ...
  draw(ctx,debug=false){
    ...
    wire([[312,450],[312,493],[450,493]],this.plate.active||this.phase!=='ENTRY');
    wire([[572,568],[572,390],[535,390],[535,370]],powered);
    wire([[760,305],[760,330],[881,330],[881,355]],this.winch.active||this.phase==='CHARGE'||this.phase==='EXIT');
    wire([[992,580],[1068,580],[1068,480],[1165,480],[1165,510]],this.phase==='CHARGE'||this.phase==='EXIT');ctx.setLineDash([]);
    ...
    if(this.cell.state==='LOOSE'||this.cell.state==='CARRIED')this.drawCell(ctx,this.cell.x,this.cell.y);
