...
export function createUI({state,input,selected,start,resume,getStage}) {
  ...
  function renderMenu(){
    screen.innerHTML=`<div class="sheet hero"><div><span class="pill">LEVEL 1 · LOCAL CO-OP</span><h1 class="hero-logo"><img src="/assets/images/group-characters.png" alt="SYNC"></h1><div class="escape-title">ESCAPE CHAIN</div><p class="typing" aria-label="Can technology bring us back together?"><span aria-hidden="true" id="typed"></span></p>${button('start','Start Game ↗','primary')}<div class="menu-links">${button('controls','Controls','ghost')}${button('controllers','Controllers','ghost')}</div></div><div class="hero-art"><span class="spark">✦</span><div class="group-crop"><img id="group-art" src="/assets/images/group-characters.png" alt="De SYNC characters samen"></div><span class="orbit-label">YOU + ME = SYNC</span></div></div>`;
  ...
  function renderSetup(){
    if(setupStep===0){screen.innerHTML=`<div class="sheet">${heading('01 / TEAM UP','Twee spelers. Eén missie.','Alleen 2 lokale spelers op hetzelfde scherm. Ontdek, communiceer en open samen de uitgang.')}${roles()}<div class="actions">${button('back','← Hoofdmenu','ghost')}${button('next','Kies jullie characters →','primary')}</div></div>`;}
      ...
      screen.innerHTML=`<div class="sheet narrow">${heading('03 / READY TO SYNC','Samen ontdekken.','Eén energiecel. Twee sluizen. Alleen samen bereikt de stroom de uitgang.')}<div class="ready-grid">${selected.map((s,i)=>`<article class="card ready-player">${avatar(CHARACTERS[s])}<div><span class="pill">PLAYER ${i+1} / ${i===0?'EXPLORER':'TECH'}</span><h3>${CHARACTERS[s].name}</h3><p>${input.assignments[i]===null?'Keyboard':`Controller ${input.assignments[i]+1}`} · ${i===0?'Drukplaat & lier':'Energiecel'}</p></div></article>`).join('')}</div><div class="note">Alleen 2 spelers · Praat met elkaar. Extra hulp vind je tijdens het spelen in Menu → Vraag een hint.</div><div class="menu-links">${button('controls','Controls bekijken','ghost')}${button('controllers','Controllers koppelen','ghost')}</div><div class="actions">${button('back','← Characters','ghost')}${button('play','Start Stage 1 ↗','primary')}</div></div>`;
